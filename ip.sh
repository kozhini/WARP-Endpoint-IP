...
endpointyx() {
	...
	wget https://raw.githubusercontent.com/kozhini/WARP-Endpoint-IP/main/files/warp-linux-$(archAffix) -O warp
...
endpoint4() {
	...
	iplist=100
	while true; do
		temp[$n]=$(echo 8.47.69.$(($RANDOM % 256)))
		n=$(($n + 1))
		if [ $n -ge $iplist ]; then
			break
		fi
		temp[$n]=$(echo 8.39.125.$(($RANDOM % 256)))
		n=$(($n + 1))
		if [ $n -ge $iplist ]; then
			break
		fi
		...
		temp[$n]=$(echo 8.39.214.$(($RANDOM % 256)))
		n=$(($n + 1))
		if [ $n -ge $iplist ]; then
			break
		fi
